...
<html lang="ko">
 <head>
  ...
  <style>
	/*############# container ##################*/
		#container{padding-bottom:100px; background-image:url("img/blur.jpg"); }

	/*############# visual ##################*/
		#section{width:400px; margin:50px auto 0; background-color:#fff; box-shadow:3px 3px 3px rgba(0,0,0,0.3); padding:20px 40px;}

		#section h2{ text-align:center; padding-bottom:20px;}
		#section form{ font-size:13px;}
		#section form p{ padding:10px 0; margin-bottom:20px;}
		#section form label{ display:block; float:left; width:80px; text-align:center; line-height:25px;}
		#section form input:not(.button){ width:280px; height:25px; text-indent:10px;}

		#section form textarea{ width:280px; height:200px; text-indent:10px;}
		#section form label[for="content"]{ line-height:200px;}
	...
	<div id="wrap">
		...
		<?=$row['content']?>
					...
					<p id="buttonArea">
						...
						<a href="#" title="수정"><input class="button" type="button" value="수정"/></a>
...
<?php
	...
	# 현재 읽은 문서가 2번이라면 2번에 해당하는 view의 값이 올라가도록 만들기
	# 기존의 값에 view의 필드정보만 바뀜
